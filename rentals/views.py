...
def search_book_view(request):
    ...
    if search_query is not None and book_ex:
        # redirected to the detail page (rentals list of the book)
...
class UpdateRentalStatusView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        instance.save()
        messages.add_message(self.request, messages.INFO, f"{instance.book.id} was successfully updates")
...
class SelectDownloadRentalsViews(LoginRequiredMixin, FormView):
    ...
    def post(self, request, **kwargs):
        formats = dict(FORMAT_CHOICE)
        format_raw = self.request.POST.get('format')
        format = formats[format_raw]
