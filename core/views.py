...
def otp_view(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            print (f'otp:{otp}  username:{username}   secret:{otp_secret_key}  valid until {otp_valid_until}')
...
def chart_data(request):
    ...
    publisher_names_counts = [x['publisher__name__count'] for x in titles_by_publisher] #this is called list comprehension
    data.append({
        'labels': publisher_names,
        'data': publisher_names_counts,
        'description': 'book title by publisher',
        'type':'pie',
    })
